...
class TestModels(TestCase):
    ...
    def setUp(self):
        ...
        self.bug = Ticket.objects.create(
            created_by=self.user, title='bug',
            description='bug description', ticket_type='Bug')
        self.feature = Ticket.objects.create(
            created_by=self.user, title='feature',
    ...
    def test_voting_on_a_Ticket_correctly_adds_Vote(self):
        self.assertEqual(
            Vote.objects.all().filter(ticket=self.bug).count(), 0)
        Vote.objects.create(user=self.user, ticket=self.bug)
        self.assertEqual(
            Vote.objects.all().filter(ticket=self.bug).count(), 1)

    def test_contributing_to_feature_correctly_adds_Contribution(self):
        self.assertEqual(
            Contribution.objects.all().filter(ticket=self.feature).count(), 0)
        Contribution.objects.create(
            user=self.user, ticket=self.feature, amount=2)
        self.assertEqual(
            Contribution.objects.all().filter(ticket=self.feature).count(),
            1)
        self.assertEqual(
            Contribution.objects.all().filter(ticket=self.feature)[0].amount,
            2)
