...
from .views import (
    bugs, features, bug, feature, add_bug,
    add_feature, add_vote, edit_bug, edit_feature)
...
class TestViews(TestCase):
    ...
    def test_login_not_required_views_get_correct_templates(self):
        ...
        bug = Ticket.objects.create(
            created_by=user, title='bug', description='bug description',
            ticket_type='bug', status='PENDING', status_colour='dark')
        feature = Ticket.objects.create(
            created_by=user, title='feature',
            description='feature description', ticket_type='feature',
            status='PENDING', status_colour='dark')
    ...
    def test_login_required_views_redirect_to_login(self):
        ...
        self.assertRedirects(
            page1, "/accounts/login/?next=/bugs_%26_features/addbug")
        self.assertRedirects(
            page2, "/accounts/login/?next=/bugs_%26_features/addfeature")
        self.assertRedirects(
            page3, "/accounts/login/?next=/bugs_%26_features/addvote/1/")
        self.assertRedirects(
            page4, "/accounts/login/?next=/bugs_%26_features/editbug/1/")
        self.assertRedirects(
            page5, "/accounts/login/?next=/bugs_%26_features/editfeature/2/")
