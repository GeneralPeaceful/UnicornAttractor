...
def view_cart(request):
    ...
    """
    num_contributions = Contribution.objects.filter(
...
def add_to_cart(request, featureid):
    ...
    """
    if not request.POST["contribution_amount"] or request.POST[
        "contribution_amount"] == "" or float(
            request.POST["contribution_amount"]) < 1:
        ...
        if payment_form.is_valid():
            ...
            try:
                payment = stripe.Charge.create(
                    ...
                    currency="GBP",
                    description="UA Feature Contributions: " +
            ...
            if payment.paid:
                messages.success(
                    request, "You have successfully paid. Please check the " +
                    "features page regularly to see when development begins.")
                request.session["cart"] = {}
                return redirect(reverse("features"))
            else:
                messages.error(
                    request, "Unable to take payment, please try again.")
        else:
            messages.error(
    ...
    if not request.POST["contribution_amount"] \
        ...
        or float(request.POST["contribution_amount"]) < 1 \
            ...
            messages.error(
                ...
                "not exceeding £999.99")
            return redirect(reverse("view_cart"))

    if featureid in cart:
        cart[featureid]["contribution_amount"] = (
            request.POST["contribution_amount"])
